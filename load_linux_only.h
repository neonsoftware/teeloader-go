...
int load_teensy_with_options(char * mmcu_param, char *vendor_id_param, char *device_id_param, char * hex_path_param, int wait_param, int hard_reboot_param, int no_reboot_param, int verbose_param)
{
	wait_for_device_to_appear = wait_param;
	hard_reboot_device = hard_reboot_param;
	reboot_after_programming = ! no_reboot_param;
	verbose = verbose_param; 

	filename = (char*) calloc(sizeof(char), 1024);
	strncpy( (char*)filename, (char*)hex_path_param, strlen(hex_path_param)); 
	...
	if (strcasecmp(mmcu_param, "at90usb162") == 0) {
		code_size = 15872;
		block_size = 128;
	} else if (strcasecmp(mmcu_param, "atmega32u4") == 0) {
		code_size = 32256;
		block_size = 128;
	} else if (strcasecmp(mmcu_param, "at90usb646") == 0) {
		code_size = 64512;
		block_size = 256;
	} else if (strcasecmp(mmcu_param, "at90usb1286") == 0) {
		code_size = 130048;
		block_size = 256;
	} else if (strcasecmp(mmcu_param, "mk20dx128") == 0) {
					code_size = 262144;
					block_size = 1024;
	} else if (strcasecmp(mmcu_param, "mk20dx256") == 0) {
					code_size = 262144;
					block_size = 1024;
				} else {
    ...
    fprintf(stderr, "C: device_id : 0x%x\n", device_id);	//return 0;
    fprintf(stderr, "C: wait_for_device_to_appear : %d\n", wait_for_device_to_appear);	//return 0;
    fprintf(stderr, "C: hard_reboot_device : %d\n", hard_reboot_device);	//return 0;
    fprintf(stderr, "C: reboot_after_programming : %d\n", reboot_after_programming);	//return 0;
